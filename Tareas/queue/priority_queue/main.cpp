...
int menu() {
    int opcion_menu;
    cout<<"\t\t Almacenando datos en priority_queue"<< endl;
    cout<<"1 -> ANADIR (push)" << endl;
    cout<<"2 -> BORRAR (pop)" << endl;
    cout<<"3 -> VER SI ESTA VACIO (empty)" << endl;
    cout<<"4 -> LISTAR" << endl;
    cout<<"5 -> VER PRIMER ELEMENTO(Top)" << endl;
...
void insertar(priority_queue<string>& stack_data,string value) {
...
**/
void listar( priority_queue<string>& stack_data,priority_queue<string>& temp_stack) {

    while(!stack_data.empty()) {
        cout<<" -> " <<stack_data.top() <<endl;
        temp_stack.push(stack_data.top());
        stack_data.pop();
    }

    while(!temp_stack.empty()) {
        stack_data.push(temp_stack.top());
        temp_stack.pop();
    }
}
/**
...
**/
void eliminar(priority_queue<string>& stack_data,priority_queue<string>& temp_stack,string delete_value) {

    bool confirmar_eliminado = false;
    //Copiando el priority_queue
    while(!stack_data.empty()) {
        if(stack_data.top() != delete_value) {
            temp_stack.push(stack_data.top());
        } else {
...
int main() {
    priority_queue<string>datos;
    priority_queue<string>temp; //Usado en eliminar
    string nombre;
    int opcion_menu;
    ...
    do {
        ...
        case 5:
            if(datos.size() > 0)
                cout<<"Dato del Front: " <<datos.top() <<endl;
            else
